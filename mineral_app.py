...
# =============================================================================
# 1. VISUALIZATION ENGINE (X3D / X3DOM)
# =============================================================================

def get_element_color(symbol):
    """Returns a simple RGB tuple for common elements (CPK-like)."""
    colors = {
        'H': (1.0, 1.0, 1.0), 'C': (0.5, 0.5, 0.5), 'N': (0.0, 0.0, 1.0),
        'O': (1.0, 0.0, 0.0), 'F': (0.0, 1.0, 0.0), 'Si': (0.9, 0.9, 0.0),
        'P': (1.0, 0.6, 0.0), 'S': (1.0, 1.0, 0.0), 'Cl': (0.0, 1.0, 0.0),
        'Mg': (0.0, 0.5, 0.0), 'Fe': (0.8, 0.5, 0.2), 'Ca': (0.5, 0.5, 0.5)
    }
    return colors.get(symbol, (0.7, 0.7, 0.7)) # Default gray

def visualize_with_x3d(structure, height=400, unique_id="x3d"):
    """
    Generates an X3D scene string and embeds it using X3DOM.
    """
    # 1. Generate Atoms (Spheres)
    atoms_xml = ""
    for site in structure:
        x, y, z = site.coords
        el = site.specie.symbol
        r, g, b = get_element_color(el)
        
        # Scale radius slightly (0.4 Å)
        atoms_xml += f"""
        <Transform translation='{x} {y} {z}'>
            <Shape>
                <Appearance>
                    <Material diffuseColor='{r} {g} {b}' specularColor='.2 .2 .2'/>
                </Appearance>
                <Sphere radius='0.4'/>
            </Shape>
        </Transform>
    ...
    # 2. Generate Unit Cell (Lines)
    matrix = structure.lattice.matrix
    # Box corners (000, 100, 010, 001, 110, 101, 011, 111)
    # We just draw the 3 vectors from origin for simplicity, or a full box.
    # Let's draw a full bounding box of the lattice vectors.
    
    # Calculate corners of the unit cell
    corners = np.array([
        [0,0,0], matrix[0], matrix[1], matrix[2],
        matrix[0]+matrix[1], matrix[0]+matrix[2], matrix[1]+matrix[2],
        matrix[0]+matrix[1]+matrix[2]
    ])
    
    # Indices for lines (0-1, 0-2, 0-3, etc)
    ...
    coord_str = " ".join([f"{c[0]} {c[1]} {c[2]}" for c in corners])
    idx_str = " ".join([str(i) for i in indices])
    
    lines_xml = f"""
    <Shape>
        <IndexedLineSet coordIndex='{idx_str}' colorPerVertex='false'>
    ...
    # We use a localized view transform to center the camera roughly
    center = np.mean(structure.cart_coords, axis=0)
    ...
    <head>
        ...
        <style>
            x3d {{ border: 1px solid #ddd; width: 100%; height: {height}px; }}
        </style>
    </head>
    <body>
        <x3d id='{unique_id}'>
            <scene>
                <Viewpoint position='{center[0]} {center[1]} {center[2]+30}' orientation='0 0 0 1' centerOfRotation='{center[0]} {center[1]} {center[2]}'></Viewpoint>
...
class ChemicalAnalyzer:
    def __init__(self, structure):
        ...
        if len(self.structure) > 0: self._analyze()

    def _analyze(self):
        covalent_elements = set(['H', 'B', 'C', 'N', 'O', 'F', 'Si', 'P', 'S', 'Cl', 'As', 'Se', 'Br', 'I'])
        ...
        for i in range(len(self.structure)):
            ...
            if elem not in covalent_elements:
                self.cations.append(i); visited.add(i); continue
            if i not in visited:
                component = []
                stack = [i]
                visited.add(i)
...
class SlabProcessor:
    def __init__(self, slab, bulk_analyzer):
        self.slab = slab; self.bulk_analyzer = bulk_analyzer
        self.local_analysis = ChemicalAnalyzer(self.slab)
        n_bulk_centers = len(bulk_analyzer.centers)
        if n_bulk_centers == 0: self.ratio_cation = 0; self.ratio_mols = {}
        else:
    ...
    def remove_indices(self, indices_to_remove):
        ...
        if len(self.slab) > 0: self.local_analysis = ChemicalAnalyzer(self.slab)
        else: self.local_analysis = None
        ...
        for n_top in range(max_rem):
            for n_bot in range(max_rem):
                ...
                if not test.is_polar():
                    self.slab = test; self.local_analysis = ChemicalAnalyzer(self.slab)
                    return True
        return False
...
with st.sidebar:
    ...
    mode = st.radio("Miller Index Selection:", ["Single Index", "Up to Max Index", "Specific List"])
    target_indices = []
...
if uploaded_file and generate_btn:
    try:
        ...
        st.error(f"Error reading CIF: {e}"); st.stop()
    ...
    if mode == "Up to Max Index":
        ...
        try:
            ...
            target_indices = []
            for item in parsed:
                if isinstance(item, (list, tuple)) and len(item) == 3: target_indices.append(tuple(item))
            if not target_indices: raise ValueError
        except: st.error("Invalid format for index list."); st.stop()
    ...
    with st.status("Analyzing Bulk Chemistry...", expanded=True) as status:
        ...
        else: st.error("No covalent scaffold found!"); st.stop()
        status.update(label="Bulk Analysis Complete", state="complete", expanded=False)

    progress_bar = st.progress(0); status_text = st.empty(); final_slabs = []
        ...
        try:
            ...
            for raw_slab in raw_slabs:
                ...
                for form, units in raw_an.molecules.items():
                    mol_cands_map[form] = []
                    for u_indices in units: mol_cands_map[form].append([proc_slab[x] for x in u_indices])
                ...
                scaffold = proc.slab; center_z = 0.5
                ...
                best_base = None; current_base = [s for s in scaffold]
                ...
                for combo in itertools.combinations(pool_cat, needed):
                    ...
                    if not ts_slab.is_polar(): best_base = ts_slab; break
                if not best_base: continue
                
                final_sites = [s for s in best_base]; success_mols = True
                for form, ratio in proc.ratio_mols.items():
                    req_n = int(round(n_centers * ratio)); 
                    if req_n == 0: continue
                    cands = mol_cands_map.get(form, []); cands.sort(key=lambda unit: abs(unit[0].frac_coords[2] - center_z))
                    if len(cands) < req_n: success_mols = False; break
    ...
    progress_bar.progress(1.0); status_text.text("Generation Complete!")
    st.header(f"Results: {len(final_slabs)} Valid Surfaces Found")
    
    if len(final_slabs) == 0: st.warning("No valid non-polar, stoichiometric surfaces found.")
    
    zip_buffer = io.BytesIO()
    with zipfile.ZipFile(zip_buffer, "w") as zip_file:
        for i, slab in enumerate(final_slabs):
            an = ChemicalAnalyzer(slab); mult = len(slab) / len(bulk)
            dipole_z = slab.dipole[2]; is_sym = slab.is_symmetric()
            hkl_str = "".join(map(str, slab.miller_index)); fname = f"slab_{hkl_str}_id{i}.cif"
            w = CifWriter(slab); zip_file.writestr(fname, str(w))
            
            with st.expander(f"Surface ({hkl_str}) | ID: {i} | Polarity: {dipole_z:.6f} eÅ", expanded=False):
                col1, col2 = st.columns([1, 1.5])
                with col1:
                    st.write(f"**Multiplicity:** {mult:.2f}x"); st.write(f"**Symmetry:** {is_sym}")
                    st.write(f"**Dipole Z:** {dipole_z:.6f}")
                    st.download_button("Download CIF", str(w), fname, "chemical/x-cif", key=f"dl_{i}")
                with col2:
                    # === VISUALIZE USING X3D ===
                    visualize_with_x3d(slab, unique_id=f"x3d_{i}")

    if final_slabs:
        st.download_button("Download All (ZIP)", zip_buffer.getvalue(), "surfaces.zip", "application/zip", type="primary")
